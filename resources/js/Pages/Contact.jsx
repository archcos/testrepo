...
export default function ContactUs() {
  ...
  const handleSubmit = (e) => {
    ...
    // Use post with proper error handling
    post('/contact', {
      preserveScroll: true,
      onSuccess: (page) => {
        reset();
      ...
      onError: (errors) => {
        // Handle rate limit errors (429 response)
        if (errors.rate_seconds || errors.rate_limit) {
          setRateLimitMessage(errors.rate_limit || 'Please wait before sending another message.');
          setRateLimitTimer(errors.rate_seconds || 60);
        }
        // Other validation errors are handled by Inertia
      },
      onFinish: (visit) => {
        // Close overlay after everything settles
        setTimeout(() => {
          setShowProcessing(false);
        }, 500);
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
              ...
              {(pageErrors?.message || Object.keys(errors).length > 0 || Object.keys(localErrors).length > 0) && (
